...
describe('acrobits.buildAccountXml', () => {
  ...
  it('renders the Acrobits Account XML with all required fields', () => {
    ...
    // Direct SIP registration to Telnyx: domain/port/transport pinned, no SRTP.
  ...
  it('omits SMS web-service URLs and rewriting rules (voice-only Account XML)', () => {
    const xml = acrobits.buildAccountXml(params);
    // Reverted to the known-good voice-registration XML — no SMS additions.
    expect(xml).not.toContain('<genericSmsSendUrl>');
    expect(xml).not.toContain('<genericSmsFetchUrl>');
    expect(xml).not.toContain('<httpMessaging>');
    expect(xml).not.toContain('<rewriting>');
  });
